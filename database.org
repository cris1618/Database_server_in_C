...
#+PROPERTY: header-args:C :main yes :includes <stdio.h> <stdbool.h> <stdlib.h> <string.h> <arpa/inet.h> <unistd.h> <assert.h> <errno.h> <sys/socket.h> <netinet/in.h> :results output :noweb yes
...
#+begin_src C
  ...
  void mainloop(int16 port){
    ...
    sock.sin_port = htons(PORT); // htons 
    sock.sin.addr.s_addr = inet_addr(HOST); 
    ...
    if (bind(s, (struct sockaddr *)&sock, sizeof(sock))) // Bind a name to a socket
      assert_perror(errno); // Quit the program

    listen(s, 20); // Accept connections
  }
  int main(int, char**);
